...
## 项目结构
系统采用“分层+模块化”组织方式：
- 核心层：解析引擎、分块器、加载器、合并器、增量解析器
- 模型层：请求/结果/文档数据模型
- 提供商层：抽象基类与多提供商实现，工厂统一创建
- 接口层：CLI命令与FastAPI服务
- 配置层：Pydantic设置与环境变量绑定
...
- [request.py](file://api-doc-parser/src/api_doc_parser/models/request.py#L8-L57)
...
subgraph "CLI/Web"
CLI["CLI<br/>cli.py"]
WEB["FastAPI<br/>api.py"]
end
ENGINE["解析引擎<br/>core/parser.py"]
CHUNK["分块器<br/>core/chunker.py"]
LOAD["加载器<br/>core/loader.py"]
MERGE["合并器<br/>core/merger.py"]
INC["增量解析器<br/>core/incremental.py"]
PROV["提供商工厂<br/>providers/factory.py"]
BASEP["提供商基类<br/>providers/base.py"]
...
图表来源
- [incremental.py](file://api-doc-parser/src/api_doc_parser/core/incremental.py#L29-L150)
- [parser.py](file://api-doc-parser/src/api_doc_parser/core/parser.py#L46-L128)
...
- [merger.py](file://api-doc-parser/src/api_doc_parser/core/merger.py#L98-L135)
...
章节来源
- [cli.py](file://api-doc-parser/src/api_doc_parser/cli.py#L246-L296)
- [api.py](file://api-doc-parser/src/api_doc_parser/api.py#L97-L112)
- [api.py](file://api-doc-parser/src/api_doc_parser/api.py#L138-L146)
