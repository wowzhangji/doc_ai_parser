...
- [core/__init__.py](file://src/core/__init__.py)
...
- [providers/__init__.py](file://src/providers/__init__.py)
...
## 项目结构
项目采用"分层+功能域"混合组织方式，新的模块化架构清晰分离了各功能域：

- 核心层（src/core/）：文档加载、智能分块、解析引擎、结果合并、增量更新
- 模型层（src/models/）：请求/结果/文档数据模型
- 提供商层（src/providers/）：LLM提供商抽象与工厂
...
subgraph "提供商层 (src/providers/)"
ProvBase["提供商基类<br/>base.py"]
ProvFactory["提供商工厂<br/>factory.py"]
end
subgraph "工具层 (src/utils/)"
Fingerprint["指纹工具<br/>fingerprint.py"]
end
subgraph "配置与初始化"
Config["配置管理<br/>config.py"]
InitCore["核心初始化<br/>core/__init__.py"]
InitModels["模型初始化<br/>models/__init__.py"]
InitProviders["提供商初始化<br/>providers/__init__.py"]
end
CLI --> Parser
API --> Parser
Parser --> Loader
...
- [core/__init__.py](file://src/core/__init__.py#L1-L19)
- [models/__init__.py](file://src/models/__init__.py#L1-L25)
- [providers/__init__.py](file://src/providers/__init__.py#L1-L23)
...
- [api.py](file://src/api.py#L76-L371)
...
**图表来源**
- [api.py](file://src/api.py#L76-L300)
- [parser.py](file://src/core/parser.py#L46-L128)
...
- [api.py](file://src/api.py#L194-L209)
- [parser.py](file://src/core/parser.py#L160-L200)
- [base.py](file://src/providers/base.py#L112-L143)

## 结论
本架构以"模块化 + 工厂 + 抽象基类"为核心设计原则，实现了对多格式文档的智能解析与多提供商的灵活适配。通过智能分块、并发解析与结果合并，系统在准确性和吞吐量之间取得良好平衡；通过增量解析进一步提升大规模文档与频繁更新场景的效率。新的模块化架构（src/core/、src/models/、src/providers/、src/utils/）使代码结构更加清晰，职责分离更加明确，便于维护和扩展。建议在生产环境中引入Redis任务队列、结构化日志与指标监控体系，以增强可观测性与可靠性。
