...
## 更新摘要
**所做更改**
- 更新了PDF加载器的表格提取机制，从pdfplumber迁移到更稳定的实现
- 完善了Word文档加载器的表格处理逻辑
- 增强了Excel加载器的DataFrame处理能力
- 优化了API端点检测算法，支持更多格式
- 更新了文档加载器的错误处理机制
- 完善了Markdown格式支持
...
**图表来源**
- [parser.py](file://src/core/parser.py#L63-L128)
...
- [loader.py](file://src/core/loader.py#L80-L152)
...
- [parser.py](file://src/core/parser.py#L13-L15)
- [chunker.py](file://src/core/chunker.py#L7-L7)
...
- [parser.py](file://src/core/parser.py#L99-L127)
...
- 示例路径参考
  - 新类定义位置：[loader.py](file://src/core/loader.py#L17-L328)
  - 注册映射位置：[loader.py](file://src/core/loader.py#L313-L327)
  - 模型结构参考：[document.py](file://src/models/document.py#L42-L75)
...
### API端点检测增强功能
- 支持的API模式
  - REST API：GET、POST、PUT、DELETE、PATCH 方法
  - URL模式：HTTP/HTTPS链接
  - 明确声明：Endpoint: 关键字
- 检测精度提升
  - 改进的正则表达式匹配
...
- [document.py](file://src/models/document.py#L8-L17)

### Markdown格式支持
- TextLoader 现在支持 Markdown 文件（.md 扩展名）
- 自动识别标题、代码块、列表等结构
- 保持与标准Markdown语法兼容
