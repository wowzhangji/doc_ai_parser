...
## 简介
本文件围绕文档模型与分块模型，系统阐述 Document 与 Chunk 的设计理念、数据结构、关键字段及其用途；说明文档元数据、内容结构、分块信息的组织方式；记录文档类型识别、内容提取与结构化存储的实现；给出不同文档格式（PDF、Word、Excel、TXT、MD）的处理差异与特殊字段；提供模型创建、操作与访问示例；解释文档指纹计算与去重机制；并阐明文档模型在整个解析流程中的作用及与各组件的交互关系。
...
### DocumentSection 类型体系
- SectionType 枚举包含：标题、段落、表格、代码、列表、API端点、未知
- DocumentSection 通过 type/content/level/metadata/children 组织章节树
- 加载器在解析过程中识别标题、API端点、代码块等，并构造相应章节

章节来源
- [document.py](file://api-doc-parser/src/api_doc_parser/models/document.py#L8-L28)
...
- [parser.py](file://api-doc-parser/src/api_doc_parser/core/parser.py#L138-L149)
...
章节来源
- [loader.py](file://api-doc-parser/src/api_doc_parser/core/loader.py#L313-L328)
- [parser.py](file://api-doc-parser/src/api_doc_parser/core/parser.py#L99-L128)
