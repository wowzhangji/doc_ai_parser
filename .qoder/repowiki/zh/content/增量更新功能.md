...
- [__init__.py](file://src/core/__init__.py)
</cite>

## 更新摘要
**所做更改**
- 更新了增量解析器的实现细节，包括指纹计算和变更检测算法
- 新增了基于分块指纹映射的增量提取优化
- 完善了全量重解析判定机制
- 更新了使用模式和示例路径
- 增强了性能考量和故障排查指南
...
- 增量解析器（IncrementalParser）
  ...
  - 关键方法：detect_changes、merge_incremental_results、should_full_reparse、compute_document_fingerprint、compute_chunk_fingerprint、create_chunk_fingerprint_map、extract_unchanged_data
- 智能分块器（SmartChunker）
...
- 指纹映射优化
  - 提供create_chunk_fingerprint_map方法创建指纹到索引的映射
  - 支持extract_unchanged_data方法提取未变更数据（可重写以实现更精细的增量提取）
...
LoadFp --> CreateMap["创建指纹到索引映射"]
CreateMap --> Compare["逐个计算新分块指纹并对比"]
Compare --> IsUnchanged{"命中旧指纹?"}
IsUnchanged --> |是| MarkUnchanged["标记未变更并记录指纹"]
IsUnchanged --> |否| MarkChanged["标记变更"]
MarkUnchanged --> ExtractData["提取未变更数据可选优化"]
ExtractData --> End(["结束"])
MarkChanged --> End
FullReparse --> End
```
...
**图表来源**
- [incremental.py](file://src/core/incremental.py#L29-L175)
...
- CLI增量更新
  ...
  - 示例路径：[cli.py](file://src/cli.py#L170-L210)
- Web服务同步解析
  - 通过POST /parse/sync传入requirement_doc与可选previous_result
  - 示例路径：[api.py](file://src/api.py#L177-L255)
- 异步任务解析
  - 通过POST /parse创建任务，后台任务中构建ParseRequest并执行解析
  - 示例路径：[api.py](file://src/api.py#L302-L352)

**章节来源**
- [cli.py](file://src/cli.py#L170-L210)
- [api.py](file://src/api.py#L177-L255)
- [api.py](file://src/api.py#L302-L352)
...
- [parser.py](file://src/core/parser.py#L10-L16)
- [api.py](file://src/api.py#L13-L21)
- [cli.py](file://src/cli.py#L1-L210)
...
- 指纹映射优化
  - create_chunk_fingerprint_map提供O(1)的指纹查找，提升大文档增量处理性能

**章节来源**
- [parser.py](file://src/core/parser.py#L130-L169)
- [parser.py](file://src/core/parser.py#L296-L304)
- [incremental.py](file://src/core/incremental.py#L177-L209)
- [chunker.py](file://src/core/chunker.py#L166-L201)
...
- 增量结果为空或不完整
  ...
  - 确认新旧文档分块策略一致（相同配置）
  - 参考路径：[incremental.py](file://src/core/incremental.py#L76-L88)
- 变更检测误判
  - 调整分块大小与重叠，确保关键信息不被截断
  - 参考路径：[chunker.py](file://src/core/chunker.py#L13-L27)
- LLM调用失败
  - 查看ParseMetadata中的failed_chunks与warnings
  - 检查网络、API Key、模型可用性
  - 参考路径：[parser.py](file://src/core/parser.py#L99-L113)
- Web/CLI接口异常
  - 检查文件类型检测与大小限制
  - 参考路径：[api.py](file://src/api.py#L94-L113)、[api.py](file://src/api.py#L202-L210)
- 指纹不匹配问题
  - 确保使用相同的分块配置（chunk_size、chunk_overlap）
  - 检查文档编码和空白字符处理
  - 参考路径：[fingerprint.py](file://src/utils/fingerprint.py#L9-L31)

**章节来源**
- [incremental.py](file://src/core/incremental.py#L76-L88)
- [chunker.py](file://src/core/chunker.py#L13-L27)
- [parser.py](file://src/core/parser.py#L99-L113)
- [api.py](file://src/api.py#L94-L113)
- [api.py](file://src/api.py#L202-L210)
- [fingerprint.py](file://src/utils/fingerprint.py#L9-L31)
...
- detect_changes
  - 输入：旧文档、新文档、旧解析结果
  - 返回：(变更分块列表, 未变更分块索引列表)
  - 参考路径：[incremental.py](file://src/core/incremental.py#L29-L74)
- merge_incremental_results
  - 输入：旧结果、新分块结果、未变更索引、总分块数
  - 返回：合并后的ParseResult（is_incremental=True）
  - 参考路径：[incremental.py](file://src/core/incremental.py#L90-L150)
- should_full_reparse
  - 输入：旧文档、新文档、变更阈值（默认0.5）
  - 返回：是否全量重解析
  - 参考路径：[incremental.py](file://src/core/incremental.py#L177-L209)
- compute_document_fingerprint/compute_chunk_fingerprint
  - 输入：Document/Chunk
  - 返回：指纹字符串
  - 参考路径：[incremental.py](file://src/core/incremental.py#L21-L27)
- create_chunk_fingerprint_map
  - 输入：分块列表
  - 返回：指纹到索引的映射字典
  - 参考路径：[incremental.py](file://src/core/incremental.py#L152-L160)
- extract_unchanged_data
  - 输入：旧结果、未变更索引、指纹映射
  - 返回：未变更数据（可重写实现更精细提取）
  - 参考路径：[incremental.py](file://src/core/incremental.py#L162-L175)
- ParseRequest.previous_result
  - 用于增量更新的前置解析结果
  - 参考路径：[request.py](file://src/models/request.py#L56-L56)

**章节来源**
- [incremental.py](file://src/core/incremental.py#L21-L209)
- [request.py](file://src/models/request.py#L51-L57)