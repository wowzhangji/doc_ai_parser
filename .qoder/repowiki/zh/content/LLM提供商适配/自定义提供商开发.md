...
## 更新摘要
**变更内容**
- 新增自定义OpenAI协议提供商的详细实现指南
- 新增自定义Anthropic协议提供商的详细实现指南
- 更新工厂模式以支持新的自定义提供商
- 增强JSON解析鲁棒性的技术细节
- 添加完整的开发流程和测试策略
...
- [request.py](file://src/models/request.py#L24-L57)
- [document.py](file://src/models/document.py#L57-L75)
...
- [custom_openai_provider.py](file://src/providers/custom_openai_provider.py#L35-L103)
...
- OpenAIProvider/AnthropicProvider
  - 使用官方AsyncOpenAI/AsyncAnthropic客户端
  - 显式设置response_format为json_object以提升JSON产出稳定性
- CustomOpenAIProvider/CustomAnthropicProvider
  - 使用httpx进行HTTP调用
  - 遵循各自协议的消息结构和认证方式
...
- [openai_provider.py](file://src/providers/openai_provider.py#L13-L82)
- [anthropic_provider.py](file://src/providers/anthropic_provider.py#L13-L82)
- [custom_openai_provider.py](file://src/providers/custom_openai_provider.py#L5-L9)
- [custom_anthropic_provider.py](file://src/providers/custom_anthropic_provider.py#L5-L9)
...
**图表来源**
- [custom_openai_provider.py](file://src/providers/custom_openai_provider.py#L87-L102)
- [custom_anthropic_provider.py](file://src/providers/custom_anthropic_provider.py#L88-L95)
- [base.py](file://src/providers/base.py#L112-L143)
...
- 参考实现路径
  - 自定义OpenAI协议：[custom_openai_provider.py](file://src/providers/custom_openai_provider.py#L12-L122)
  - 自定义Anthropic协议：[custom_anthropic_provider.py](file://src/providers/custom_anthropic_provider.py#L12-L96)
  - 工厂注册：[factory.py](file://src/providers/factory.py#L14-L71)
  - 测试参考：[test_providers.py](file://tests/test_providers.py#L13-L106)
...
- 本地运行
  - 安装依赖与激活虚拟环境
  - 设置.env环境变量
  - CLI示例：[README.md](file://README.md#L66-L91)
- Web服务
  - 启动FastAPI服务：[README.md](file://README.md#L93-L104)
- Docker/容器化
  - 建议将.env与配置文件挂载到容器内
  - 为自定义后端暴露相应端口并配置安全策略
...
**章节来源**
- [pyproject.toml](file://pyproject.toml#L25-L59)
- [custom_anthropic_provider.py](file://src/providers/custom_anthropic_provider.py#L46-L47)
...
- **OpenAI协议适配**
  ...
  - 消息格式：{"role": "system/user", "content": "..."}
  - 可选参数：temperature、max_tokens、response_format

- **Anthropic协议适配**
  - 端点：/messages
  - 认证：x-api-key: {api_key}
  - 特殊头部：anthropic-version: "2023-06-01"
  - system提示：独立的system字段而非消息数组

#### 2. 错误处理策略
- HTTP状态码异常：捕获HTTPStatusError并记录详细信息
- JSON解析异常：使用parse_json_response的多重解析策略
- 超时处理：合理设置timeout参数，避免长时间阻塞
- 重试机制：结合max_retries参数实现指数退避

#### 3. 性能优化建议
- 连接池复用：使用AsyncClient进行连接复用
- 批量处理：对多个chunk进行并发处理
- 缓存策略：对频繁访问的模型列表进行缓存
- 资源清理：确保异步资源正确释放

**章节来源**
- [custom_openai_provider.py](file://src/providers/custom_openai_provider.py#L47-L76)
- [custom_anthropic_provider.py](file://src/providers/custom_anthropic_provider.py#L43-L69)
- [base.py](file://src/providers/base.py#L112-L143)